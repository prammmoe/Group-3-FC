...
struct CardView: View {
    ...
    var body: some View {
        HStack(){
            HStack(spacing: 8){
                VStack(spacing: 2){
                    Text(borrower.nextDueDate.formatted(.dateTime.weekday().locale(Locale(identifier: "id_ID")))).font(.caption2).fontWeight(.bold).foregroundColor(.black)
                    VStack{
                        VStack{
                            Spacer()
            ...
            HStack(spacing: 12){
                VStack(alignment: .trailing){
                    ...
                    Text(abs(borrower.totalDebtAmount), format: .currency(code: "IDR")).font(.body).fontWeight(.bold).foregroundColor(Color("Primary"))
                }
